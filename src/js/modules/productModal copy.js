...
export default class ProductModal {
  ...
  constructor({ productModalSelector, triggerElements, database, spinnerSrc }) {
    ...
    this.triggers = document.querySelectorAll(triggerElements) // change
    this.db = database
    this.spinner = spinnerSrc
    this.error = false
    this.live = document.querySelector('.catalog-list') // change
    this.live2 = this.live.children // change
  }

  init() {
    for (const element of this.live2) {
